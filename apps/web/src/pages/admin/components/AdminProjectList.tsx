...
export function AdminProjectList({ selectedUser, knownUsers }: Props) {
	...
	const assignDebounced = useDebouncedCallback(
		async (project: Project) => {
			if (!selectedUser) {
				toast.info("Select a user to assign.", "admin:assign:missing-user");
				return;
			}
			...
			try {
				...
				toast.success("Assignment updated.", `admin:assign:success:${project.id}`);
			} catch (err) {
				const message = (err as { data?: string })?.data ?? "Failed to assign project";
				toast.error(message, `admin:assign:error:${project.id}`);
			}
		},
		250,
					...
					>
						...
						<div className="mt-3 flex flex-wrap items-center justify-between gap-3">
							...
							<Button
								...
								onClick={() => assignDebounced.callback(p)}
							>
								Assign to selected
							</Button>
						</div>
