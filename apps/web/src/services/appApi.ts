...
const zodErrorToMessage = (error: z.ZodError) =>
...
export const appApi = createApi({
	...
	endpoints: (builder) => ({
		...
		listUserProfiles: builder.query<
			Paginated<UserProfile>,
			ListUserProfilesArgs
		>({
			queryFn: async (args) => {
			...
			serializeQueryArgs: ({ endpointName, queryArgs }) => {
				const key = `${queryArgs.searchEmail ?? ""}-${
					queryArgs.limit ?? PAGE_SIZE_DEFAULT
				}`;
			...
			providesTags: (result) => {
				const base: { type: "UserProfile"; id: string }[] = (
					result?.items ?? []
				).map((u) => ({
					type: "UserProfile",
					id: u.id,
		...
		listProjects: builder.query<Paginated<Project>, ListProjectsArgs>({
			...
			serializeQueryArgs: ({ endpointName, queryArgs }) => {
				const key = `${queryArgs.assignedUserId ?? "ALL"}-${
					queryArgs.isCompleted ?? "ANY"
				}-${queryArgs.limit}`;
				return `${endpointName}-${key}`;
			},
			merge: (currentCache, newData, { arg }) => {
		...
		createProject: builder.mutation<Project, CreateProjectInput>({
			queryFn: async (input) => {
				...
				if (!parsedInput.success)
					return { error: apiError(zodErrorToMessage(parsedInput.error)) };
					...
					try {
						dispatch(
							appApi.util.updateQueryData(
								...
								{ offset: 0, limit: PAGE_SIZE_DEFAULT },
								(draft) => {
									draft.items = uniqueById([
										data,
										...draft.items.filter((p) => p.id !== tempId),
									]);
								}
							)
						);
		...
		updateProject: builder.mutation<Project, UpdateProjectArgs>({
			queryFn: async ({ id, patch }) => {
				...
				if (!parsedPatch.success)
					return { error: apiError(zodErrorToMessage(parsedPatch.error)) };
			...
			async onQueryStarted(
				...
				const nextAssignedUserId =
					patch.assigned_user_id ?? previousAssignedUserId;
				...
				const applyToList = (
					...
					updater: (draft: Paginated<Project>) => void
				) => {
					try {
						const pr = dispatch(
							appApi.util.updateQueryData("listProjects", args, updater)
						);
				...
				if (
					...
					applyToList(toArgs, (draft) => {
						...
						const base: Project = {
							...
							created_at:
								optimisticProject?.created_at ?? new Date().toISOString(),
							updated_at: new Date().toISOString(),
						};
